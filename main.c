...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
volatile uint8_t dataAvail = 0;			// Used to determine when a button has been pressed.
...
int main(void)
{
  ...
  uint8_t init = 0;						// Logic for the start up of the game.
  uint8_t inGame = 0;						// Logic for the start up of the game.
  uint8_t askedGameQuestion = 0;						// Logic for the start up of the game.
  ...
  uint8_t playerAlive = 0;					// Logic for when the user is in the game.
  ...
  char room[] = "Hull";						// String to change rooms.
		  ...
		  if (strcmp(room,"Captain's Cabin")==0) {		  // Room: ---Captain's Cabin---
			  if (roomEnter) {
			    ...
			    uint8_t screen[] =	"                                              \r\n"
									...
									"~~~~~~~~~~~~~~\\--:|:--:|:--:|:--:|:--:|/~~~~~~\r\n"
									"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\r\n";
				HAL_UART_Transmit(&huart2,screen,sizeof(screen),1500);
				roomEnter = 0;
			  }else if (!choice){
				   HAL_Delay(100);
							  ...
							  if (dataAvail && (rx_buff[0]=='1')) {
								  ...
								  while (1)
								  {
										...
										if (i==4) {
											attempt[4] = '\0';
											HAL_Delay(50);
											checkAns(attempt, answer, &endCode);
											HAL_Delay(50);
										    if (endCode) {
											    HAL_UART_Transmit(&huart2,line,sizeof(line),50);
			  ...
			  if (roomEnter) {
				  ...
				  if (dataAvail && (rx_buff[0]=='1')) {			// Worried Crew Member Choice
					  ...
					  items( kaleidoscope,  key,  beer,  food,  hammer,  rag,  rope,  locket);
					  dataAvail = 0;
					   ...
					   while (1)
					   {
						   if (!storageBox) {
							   ...
							   if (dataAvail && (rx_buff[0]=='1') && (!rope ^ chiRope) && (!rag ^ worRag)) {
								   ...
								   HAL_UART_Transmit(&huart2,line,sizeof(line),50);
								   uint8_t options[] = "(1) Get The Rope\r\n(2) Get The Clean Rags\r\n(3) Go Back";
								   HAL_UART_Transmit(&huart2,options,sizeof(options),50);
								   HAL_UART_Transmit(&huart2,line,sizeof(line),50);
								   dataAvail = 0;
								   while (1)
